Shader "Custom/Gaussian Blur Filter"
{
    ...
    {
        _MainTex("-", 2D) = "white" {}
        _BlurMod("Blur Mod", Vector) = (1,1,1,1)
    }
    ...
    {
        return gaussian_filter(i.uv, float2(_MainTex_TexelSize.x * _BlurMod.x, 0));
    }

    half4 frag_blur_v(v2f_img i) : SV_Target
    {
        return gaussian_filter(i.uv, float2(0, _MainTex_TexelSize.y * _BlurMod.y));
    }
